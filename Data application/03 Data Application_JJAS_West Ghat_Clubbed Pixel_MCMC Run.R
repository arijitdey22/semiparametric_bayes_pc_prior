...
#####==========================================================================================
#####==========================================================================================
#model fit with semi-parametric set up

#set the working directory and run the functions
...
reg.N.best.theta <- theta.vec[which.min(WAIC.northern[81:90])]
reg.M.best.theta <- theta.vec[which.min(WAIC.middle[81:90])]
reg.S.best.theta <- theta.vec[which.min(WAIC.southern[81:90])]

#------------------------------------
#MCMC Run

all.chain.store.clubbed.semipar <- list()
...
for (i in  1:3)
{
  ...
  cur.store <- matrix(NA, ncol = n.B.MCMC_fit + 1, nrow = mcmc.iters - burn.in)
  cur.data <- numeric()
  covariate <- numeric()
  ...
  B.semipar.MCMC_fit <- eval.basis(covariate, basis)

  #MCMC chain
  fit.observed.GE.semipar <- mcmc.GE(cur.data, B.semipar.MCMC_fit, 
  ...
  #-- -- -- -- -- -- -- -- 
  #stroing the chains
  cur.store[ ,1] <- fit.observed.GE.semipar$alpha
  cur.store[ , 2:ncol(cur.store)] <- fit.observed.GE.semipar$beta
  
  all.chain.store.clubbed.semipar[[i]] <- cur.store
  
  print(paste0("Location = ", i))
  
}

save(all.chain.store.clubbed.semipar, file = "04 Data_Store_MCMC Chains_clubbed_semipar.Rdata")
...
#run the MCMC functions

load("00 Data_Store_Raw Cleaned Data.Rdata")
load("02 Data_Store_WAIC_best.pair.Rdata")

theta.vec <- seq(2,10,2)
...
for (i in 1:3)
{
  ...
  for (j in 1:length(theta.vec))
  {
    fit.observed.GE.semipar <- mcmc.GE(cur.data, B.fit, 
                                       alpha.init = NULL, beta.init = NULL,
                                       # priors
                                       alpha.theta = theta.vec[j],
                                       beta.mean = 0, beta.sd = 100,
                                       # mcmc settings
                                       iters = 5e3, burn = 2e3, thin = 5)
    
    cur.store[[j]] <- fit.observed.GE.semipar$alpha
    
    print(paste0("Location = ", i, ", theta = ", j))
  }
  
  #-- -- -- -- -- -- -- -- 
  #calculating the WAIC
  post.vs.theta.all.store[[i]] <- cur.store
}


save(post.vs.theta.all.store, file = "07 Post.vs.Theta.all.store.Rdata")


#=======================================================================================
#####===================================================================================
#model fit with parametric set up
...
all.chain.store.clubbed.par <- list()
...
for (i in  1:3)
{
  ...
  #Setting the basis functions for MCMC run
  covariate.scaled <- scale(covariate, center = T, scale = T)
  # mean(covariate.scaled)
  # var(covariate.scaled)
  B.par.MCMC_fit <- matrix(c(   rep(1, length(covariate.scaled)), covariate.scaled), ncol = 2)
  cur.store <- matrix(NA, ncol = ncol(B.par.MCMC_fit) + 1, nrow = mcmc.iters - burn.in)
  
  #MCMC chain
  fit.observed.GE.par <- mcmc.GE(cur.data, B.par.MCMC_fit, 
                                 alpha.init = NULL, beta.init = NULL,
                                 # priors
                                 alpha.theta = alpha.theta,
                                 beta.mean = 0, beta.sd = 100,
  ...
  #-- -- -- -- -- -- -- -- 
  #stroing the chains
  cur.store[ ,1] <- fit.observed.GE.par$alpha
  cur.store[ , 2:ncol(cur.store)] <- fit.observed.GE.par$beta
  
  all.chain.store.clubbed.par[[i]] <- cur.store
...
save(all.chain.store.clubbed.par, file = "04 Data_Store_MCMC Chains_clubbed_par.Rdata")
